...
> [!NOTE]
> `--fetch-base` is only valid when the URL contains a pull request ID. Using it without a PR URL returns an error. `merge_base_sha` is computed by querying the VCS provider API (Bitbucket `fromHash`, GitHub compare, GitLab `merge_base` endpoint). The result is then materialized in the local shallow clone via go-git deepening so that `git diff --merge-base <sha>` works. If the API or go-git materialization fails, a git-binary fallback computes and materializes using the target-branch tip. If all paths fail, the field is omitted and the fetch still succeeds with `base_sha` only.
