...
**Approve or unapprove a merge request with head protection** <br>
`integration-vcs --action setStatusOfPR` accepts an optional `--require-head-sha` flag. Provide the commit you reviewed locally; if GitLab reports a different head SHA the command aborts, preventing accidental approval of updated code.

```bash
scanio integration-vcs --vcs gitlab --action setStatusOfPR --status APPROVED \
  --domain gitlab.com --namespace testing_scanio --repository testing_scanio --pull-request-id 1 \
  --require-head-sha 0123456789abcdef0123456789abcdef01234567 https://gitlab.com/testing_scanio/testing_scanio/-/merge_requests/1
```

**Retrieve merge request metadata and reviewer decisions** <br>
Use `integration-vcs --action checkPR` to export the merge request object without fetching code. The response includes a `reviewers` array where each entry represents a reviewer/approver and the highest-priority verdict GitLab currently exposes.
...
Reviewer entries include `reviewer.user_name`, `reviewer.email`, and a `verdict` such as `APPROVED`, `REJECTED`, or `PENDING`. Refer to the [Integration-VCS command reference](cmd-integration-vcs.md) for the complete JSON schema including the new reviewer field.
