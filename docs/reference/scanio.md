...
| PR diff path    | `{home_folder}/tmp/<VCS_domain>/<namespace_name>/<repository_name>/pr/<pr_id>/diff` | Diff-mode artifacts (e.g., sparse files from `--diff`) are stored here; the full repository checkout lives under `projects_folder`. |
